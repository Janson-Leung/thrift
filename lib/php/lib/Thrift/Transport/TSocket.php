...
class TSocket extends TTransport {
	...
	public function open() {
		if ($this->isOpen()) {
			throw new TTransportException('TSocket: Socket already connected', TTransportException::ALREADY_OPEN);
		}

		if (empty($this->host_)) {
			throw new TTransportException('TSocket: Cannot open null host', TTransportException::NOT_OPEN);
		}

		if ($this->port_ <= 0) {
			throw new TTransportException('TSocket: Cannot open without port', TTransportException::NOT_OPEN);
		}
		...
		if (@socket_connect($this->handle_, $this->host_, $this->port_) === false) {
			$this->error("TSocket: Failed to connect socket to {$this->host_}:{$this->port_}");
		}

		if (@socket_set_option($this->handle_, SOL_SOCKET, SO_RCVTIMEO, array('sec'=>$this->recvTimeoutSec_, 'usec'=>$this->recvTimeoutUsec_)) === false) {
			$this->error("TSocket: Failed to set socket stream recv timeout option");
		}

		if (@socket_set_option($this->handle_, SOL_SOCKET, SO_SNDTIMEO, array('sec'=>$this->sendTimeoutSec_, 'usec'=>$this->sendTimeoutUsec_)) === false) {
			$this->error("TSocket: Failed to set socket stream send timeout option");
		}
	}
	...
	public function read($len) {
		...
		if ($data === false) {
			$this->error("TSocket: Failed to read data from {$this->host_}:{$this->port_}");
		}
	...
	public function write($buf) {
		...
		while($written < $length) {
			...
			if ($fwrite === false) {
				$this->error("TSocket: Failed to write buffer to {$this->host_}:{$this->port_}");
			}
